...
Audit of how the generated LVGL bindings store Python objects and whether a
MicroPython-family GC can collect them prematurely. Applies to both `lvmp.c`
and `lvcp.c`, which share the emission in `binding/emit_c.py`.
...
- **Spike module** (`lvgl-circuitpython/src/circuitpython_spike/shared-bindings/lvgl/__init.c`) only registers
  `init` / `deinit`; generated symbols live in `lvcp.c` and are merged via `LVCP_MODULE_GLOBALS`.
- **Allocator** (`lvgl-circuitpython/src/lv_mem_core_circuitpython.c`): LVGL heap allocations use `gc_alloc` / `m_malloc`;
...
See also the
[CircuitPython spike notes](https://github.com/PyDevices/lvgl-circuitpython/blob/main/docs/circuitpython-spike.md#open-questions).
